...
本服务器实际使用的卡为 **NVIDIA A800-SXM4-80GB**（Ampere、A100 同代计算能力）。GPU 7 在两次
preflight 中均为 0% 利用率、0 MiB 进程显存后，已完成 A1 的 CPU/GPU 成对详细放置测试，以及 A1/A4
的端到端 CUDA 测试。CUDA 源码在 `sm_80` 配置下编译并通过测试；所有运行时数字均在后文按当前
Abacus/reverse 合法化基线记录。
...
`116,932,383.577`，当前 Abacus/reverse 合法化后的 HPWL 是 `123,505,082`。这是约 5.62% 的落差；如果只展示前者，
就会把最该解决的质量损失藏起来。
...
|---|---|---|---|
| 1. warp 级精确窗口详细放置 | 一个窗口一个 CUDA block；24 个线程各评估一个四单元排列，硬件一次批量给出 argmin | 已实现；当前 A1 160 轮已完成 CPU/GPU 合法化后配对。RUDY 组合须按新合法化基线复跑 | **主方案** |
...
### A1：当前 Abacus/reverse 合法化基线下的窗口对照

此前的普通行级合法化实验保留在诊断目录作为历史证据，但当前源码的 Abacus/reverse 合法化器已显著
降低合法化损失，不能把两代绝对 HPWL 混在一张表内。以下是当前代码、quadratic initial、`seed=2026`、
160 轮、同一 CPU 全局布局和同一合法化起点 `123,505,082` 的可比较结果：

| 最后阶段 | 最终 HPWL | 相对合法化起点 | 详细阶段时间 | 枚举排列数 | 合法 |
|---|---:|---:|---:|---:|---|
| 无详细放置 | 123,505,082 | — | — | — | 是 |
| 4-cell / CPU | 122,082,894 | -1.1515% | 2.865 s | 1,960,514 | 是 |
| **4-cell / A800 GPU 7** | **122,166,164** | **-1.0841%** | **0.580 s** | **1,960,514** | 是 |
| 5-cell / CPU | 121,951,537 | -1.2579% | 14.047 s | 7,413,214 | 是 |

四单元已经获得五单元总改善量的 **91.54%**，只花其 **20.39%** 的 CPU 详细阶段时间。由四到五单元
仅额外减少 131,357 HPWL（0.1064 个百分点），却额外增加 11.182 秒、候选数增长到约 3.78 倍。因此
`window=4, passes=2` 仍是默认 GPU 质量/吞吐甜点；五单元保留为允许更长离线时间时的绝对 QoR 选项。

同一起点的 CPU/GPU 四单元配对尤为重要：GPU 详细阶段比 CPU **快 4.94×**，最终 HPWL 仅高
**0.0682%**。GPU 并发评分会让跨窗口 net 的候选顺序与 CPU 串行参考略有不同，但 CPU 的精确接受门槛
确保两条路径都严格改善合法化起点，且二者均为零 overlap/off-row/unplaced。

### A1：端到端 CUDA 实测

在同一输入、seed 和 160 轮预算下，GPU 全局路径因浮点归约与控制器接受了 141 步（CPU 为 140 步），
所以不应要求逐比特相同；应比较最终合法质量、课程密度和分段运行时。

| 阶段 | CPU 参考 | A800 GPU 7 | 加速 |
|---|---:|---:|---:|
| 全局布局 | 35.193 s | 6.127 s | 5.74× |
| 合法化 | 0.829 s | 0.819 s | 1.01× |
| 四单元详细放置 | 2.865 s | 0.593 s | 4.83× |
| 三个阶段合计 | 38.886 s | 7.540 s | **5.16×** |
| 最终合法 HPWL | 122,082,894 | **121,063,924** | **-0.8347%** |

GPU 结果的最终课程归一化溢出率为 `0.0451028`，CPU 为 `0.0442364`，均明显满足 0.10 门槛；两者均
`legal=true`。GPU 全局显式数据预算为 78,925,844 bytes，详细阶段为 36,904,916 bytes，远低于本次
2 GiB 上限。

### A4：280 轮端到端 A800 扩展验证

A4（494,716 movable modules、1,912,420 pins）在 GPU 7 上完整完成 280 轮预算：全局布局 24.771 s、
Abacus 合法化 3.342 s、四单元详细放置 1.541 s，三个阶段合计 29.654 s。详细阶段从
1,142,150,298 降至 1,132,194,492（-0.8717%），考察 4,750,086 个排列；最终
`legal=true`、overlap/off-row/unplaced 均为零，课程归一化溢出率为 `0.0432624`。

全局 CUDA 显式预算为 166,361,880 bytes，详细 CUDA 预算为 78,697,984 bytes，任务结束后 GPU 7 回到
0 MiB 进程显存。当前 A4 的 CPU/GPU 详细阶段成对速度测试可作为后续扩展，但 A1 已经完成同一起点配对，
而 A4 已验证了实际大规模端到端可运行性。

### RUDY 路径的版本边界

RUDY 的 V2 参数筛选、留出栅格和合法化后验证仍完整保存在
[RUDY 实验记录](rudy_experiments.md)，它支撑“多 GPU Pareto 多起点”的候选方向。由于当前合法化器已升级，
其中的旧绝对 HPWL 不与本节 Abacus 基线混比；若把 RUDY 作为当前正式组合配置，应按同一新基线重新跑
CPU/GPU 配对并保留 128×128 留出指标。

## CUDA 实现、资源边界和已完成的 GPU 证据
...
- GPU 1--4 或 7 的编号限制与全局 CUDA 后端一致；启动前保留至少 4 GiB 空闲显存，所有显式数组计入独立预算；
...
scripts/run_detailed_gpu_study.sh build-cuda \
  ...
  out/diagnostics/a100-detailed-placement/a1-window4-cuda-160-gpu7 7
```

即使显存充足，利用率高的卡也会被该脚本拒绝；此次 GPU 7 在连续两次 0% 利用率检查后才启动。
现有 GPU 证据已满足：

1. A1 同起点 CPU/GPU 四单元配对中，GPU 详细阶段 4.94× 更快，QoR 差异仅 0.0682%；
2. A1 和 A4 端到端 CUDA 路径均 `legal=true`、overlap/off-row/unplaced 均为零；
3. 两次运行都明确记录了 `detailed_compute_backend_used=cuda`、`detailed_cuda_device_used=7` 和独立预算峰值；
4. A4 已完成 280 轮大规模端到端验证；若需要报告 CPU/GPU 的 A4 详细阶段精确倍率，再补同起点 CPU 对照即可。

因此当前严谨结论是：**CUDA 详细放置已完成编译、测试、A1 同起点 QoR/速度配对和 A1/A4 端到端实测。**
它不是尚未运行的构想；其余工作是增加 seed、RUDY 组合和 A4 CPU 成对统计的稳健性，而非补足基本可用性。
